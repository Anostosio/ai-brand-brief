...
export function buildLocalDraft(rawData) {
  ...
  if (data.language === 'ru') {
    ...
    return {
      ...
      positioning: `${data.brandName} должен занимать понятную позицию в категории за счёт следующего отличия: ${data.differentiator || 'требуется уточнить подтверждаемое отличие бренда'}.`,
      ...
      messages: [
        ...
        `Подход строится вокруг реального контекста аудитории: ${data.audience}.`,
        `${data.differentiator || 'Ключевое отличие бренда необходимо проверить и сформулировать точнее.'}`
      ],
      visual: `Визуальная система должна передавать качества «${traits.slice(0, 3).join(', ') || 'ясность, уверенность, человечность'}» через типографику, композицию, ритм, изображения и повторяемые элементы. ${data.avoid ? `Избегать: ${data.avoid}.` : 'Отдельно определить визуальные клише категории, которых следует избегать.'}`,
      deliverables: data.deliverables || 'Уточнить список носителей, форматов и технических требований до начала дизайн-концепции.',
      risks: `${data.proof ? `Опорные факты: ${data.proof}.` : 'Пока не указаны подтверждения ценности и отличий бренда.'} Не считать предположения о рынке и аудитории доказанными без исследования.`,
      nextSteps: [
  ...
  return {
    ...
    positioning: `${data.brandName} should earn a clear place in its category through this stated difference: ${data.differentiator || 'a defensible brand difference still needs to be defined'}.`,
    ...
    messages: [
      ...
      `Built around the real context of ${data.audience}.`,
      `${data.differentiator || 'The key brand difference must be validated and articulated more precisely.'}`
    ],
    visual: `The visual system should express ${traits.slice(0, 3).join(', ') || 'clarity, confidence and humanity'} through typography, composition, rhythm, imagery and repeatable assets. ${data.avoid ? `Avoid: ${data.avoid}.` : 'Identify category clichés that the system should avoid.'}`,
    ...
    risks: `${data.proof ? `Available proof: ${data.proof}.` : 'No proof points have been supplied yet.'} Do not present assumptions about the market or audience as research findings.`,
    nextSteps: [
